...
datatable(head(data_sel), caption = 'Table 1: This is a simple caption for the table.')
...
## 7. Heatmap to show I and II wave
...
ggplot(data=new_data_agg, aes(x= total_cases_mln, y = reorder(location, total_cases_mln), fill=continent)) +
  ...
  labs(title = 'Which are top 10 countries by Total Cases?',
...
#gganimate::animate(gifplot, duration = 10, fps = 20, renderer = gifski_renderer())
anim_save("plot1.gif")
anim_save("plot2.gif")
...
plot3 <- ggplot(check, aes(Variables, P_Value, fill = P_Value)) +
  geom_col() +
  ...
  theme(
    ...
    panel.ontop = TRUE
  ) + transition_states(Variables, wrap = FALSE) +
  shadow_mark() +
  enter_grow() +
  enter_fade()
plot3
