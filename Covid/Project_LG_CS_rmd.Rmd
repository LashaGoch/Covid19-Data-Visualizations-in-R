...
datatable((data), caption = 'Table 1: This is a simple caption for the table.', class = 'cell-border stripe', options = list(pageLength = 5))
```


```{r}
temp <- data.frame(data) %>% select(continent, location, gdp_per_capita,
                                    population_density, total_cases, population)
...
a <- ggplot(data_agg) + 
  geom_point(aes(x = total_cases, y = population, color = continent, 
                 size = gdp_per_capita)) +
    labs(x = 'Total Cases',
       y = 'Population',
       title = "Covid total cases vs population",
       caption = 'source: ') +
...
ggplot(data = data_agg, aes(x = total_cases)) +
...
ggplot(iris, aes(x = Sepal.Width, fill = Species)) + 
  geom_histogram(data = iris[,-5], fill = "grey", alpha = .5, bins = 30) + 
  geom_histogram(colour = "black", bins = 30) +
  facet_wrap(~ Species) + guides(fill = FALSE) 
...
nice <- ggplot(data_anim, aes(total_cases, total_deaths,
