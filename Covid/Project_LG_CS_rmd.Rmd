...
#head(data_agg)
knitr::kable(data_agg[data_agg$location == "Zimbabwe",], capture = "Checking record for Zimbabwe")
...
data_for_dist <- data_agg %>%
  count(continent, gdp_per_capita_group)

data_for_dist <-  na.omit(data_for_dist)

datatable(head(data_for_dist), caption = 'Table 1: This is a simple caption for the table.')



ggplot(data_for_dist, aes(x = continent, y = gdp_per_capita_group)) + 
  geom_tile(aes(fill = n), color = 'white', show.legend = F) +
  theme_minimal() + 
  geom_text(aes(label = n), size = 5, fontface = 'bold', color = 'white') +
  labs(title = 'Distribution of countries by GDP_per_capita groups & continents') +
