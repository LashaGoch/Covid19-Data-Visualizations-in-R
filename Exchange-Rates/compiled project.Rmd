...
temp <- data.frame(data) %>% dplyr::select(continent, location, gdp_per_capita,
...
#datacove <- read_csv("https://raw.githubusercontent.com/LashaGoch/Visualizations-in-R/main/Covid/time_series_covid19_confirmed_global.csv?token=ANWKYHVL5ZI2CLRONZEAV2TADRE7O")
head(datacov)
```
...
temp3 <- temp2 %>%
    group_by(date, continent) %>% dplyr::summarize(Mean_ex = mean(Exchange_rate, na.rm=TRUE),  sum_cases_pm= sum(total_cases_per_million, na.rm = TRUE))
