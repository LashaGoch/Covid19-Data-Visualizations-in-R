...
```
## 14. Loading Exchange Rate Data

We will load the exchange rate data and a mappig file of country with country code so we can join it with the covid data using the Country Code and Date.
The exchange rate data is in terms of Euro meaning that in terms of all currency are valued with the base as Euro; for example if exhange rate on a given date for Indian Rupees is 63, it means that on that day, 1 Euro = 63 Indian Rupees.
First we load the data and take only the relavent columns and do data type transformations:
...
exchange_rate_euro <- read.csv("eurofxref-hist/eurofxref-hist.csv", stringsAsFactors=FALSE)
...
```
Next,lets load the mapping file merge it with out covid data:
```{r}
...
head(data_sub2)
```
## 15. Animated Line Chart of Exchange Rate Trend by Continent

Here, we will take the average or mean per continent and try to map how they varied during the spread of Covid. This will help us in determing if the Covid affect the exchange rate or not. As we can see from the graph, the continent with the most erratic is Asia, which is line with the cases we on=bserver there because China and India combined had had the largest cases so far in the world. Next comes Africa and North America. Europe has been erractic in the start, but then more or less remained stable.
```{r}
library(hrbrthemes)
library(viridis)
library(ggplot2)
...
```
## 16. Animated Trend Line Chart of Covid Cases per Capita

In this visual, we want to not only see how the Covid cases progressed overtime, but specifically see how they varied at a per capita level. 
As you can see Asia, despite high risk countries like India and China is at the bottom because of their populations. Secondly, since October 2020, seems to be at the top, which can maily be becaus eof the less population and the second wave in Europe.
...
```
# 17. Interactive Tree Map of Exchange Rate Fluctuation

Here we provide a different view of how much fluctuation did the Exchage rates of different countries go through overtime across continents in the time period we are observing. In the time period observed, we display the distinct shares of the exchange rate, on any given day, in terms of the maximum value (or the most devalued exchange rate value) that was observed in the time period.The higher the ratio on any given day, the more devalued the currency of that country was on that day. We have the distinct values that were observed in the time period that we are observing, but when we drill down to country level, we see the distinct values in a shape that is insicative of the duration for which that particular value(or share of exchange rate in terms of maximum value) prevailed. 
This is a more aggregated view as opposed to the trend that we were observing for each day earlier.But, the more the numberof square, the more fluctuation that happened within that particular continent/country.
Also, the shade and size is indicative of the value, specified for each country. So the larger the size of the square the higher fluctuation the continent/country went through. 
The grapg is very 
...
tree_df <- temp2 %>% group_by(continent, location) %>% mutate(Norm = round(Exchange_rate/max(Exchange_rate),2)) %>% dplyr::select(continent, location, Norm)
...
```
#18. Data Preparation for Regression and Doing a Simple Regression

Here we are just preparing the Data that we would require for doing a simple regression by doing the required merges and joins.
...
Then we do a very simple regression using the lm function. For noww we are doing a simple regression a we want to provide a quantitaive perspective that we are trying to analyze visually.
As we can see from the model the p-value indicates that the number of covid cases is a significant indicator of the exchange rate and we also observe that the Covid cases cofficient is positive, meaning, the higher the value of the the cases (its normalised with population so we interpret it as the higher the value of the cases per capita), the higher the value of the exchange rate (meaning the more fluctuations are observed in the exchange rate of that continent).  
But on an overall level, the R- square of the model is pretty low, indicating that the variables explain only 14% of the variation that we observe in the fluxtuation of the exchange rates. This is true because there are many other factors on which Exchange rate depends and we have not taken them as of now.
```{r}
library(moonBook)
library(jtools)
...
# 19. How does Exchange Rate differ in a Continent with given no. of Covid Cases

Here we are trying to display the commanity that is observed in all the continents when Covid cases rise: the Currency of the countries on that continent go through a devalution , meaning that Covid has a negative impact in the country's currency's value. As we see, in all the bars, the shade is darker on the top parts of them, indicating a high value of the exchange rate, further confirming that higher cOvid cAses is accompanied by a  more devalued currency for a given continent.
```{r}
...
# 20. Regrression Fit 1
As we can see fromthe graph, the model is a linear model which predicts a linear rise between Covid cases and Exchange Rate.
...
# 21. Regression Fit 2
In this graph, we have also fit of the model, but with data pont displayed. As you can see, Europe us limited to the top of the graph beacse of the exteme traned that was observed both in its exchange rate and covid cases per capita and day.
Asia also has some outlier, but those are mostly in cases were covid cases are comparitively low and so is the exchange rate (meaning the more valued the currency of that country is). Th other continents are still closer to the model line.
